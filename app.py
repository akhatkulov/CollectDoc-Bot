...
async def start(message: telebot.types.Message):
    if message.text == "/start": #and await join(message.chat.id)
        await bot.send_message(message.chat.id, "<b>Salom, siz qaysi yo'nalish uchun hujjat topshirmoqchisiz?</b>",reply_markup=home_keys())
        try:
...
async def more(message: telebot.types.Message):
    ...
    if step == "channel_del" and message.text != "/start" and message.text != "/admin":
        try:
    ...
    if step == "add_channel" and message.text != "/start" and message.text != "/admin":
        if await put_channel(message.text):
            await bot.send_message(message.chat.id, f"{message.text} kanali qabul qilindi!")
        else:
            await bot.send_message(message.chat.id,
                                   "Xatolik! Bu kanal oldin qo'shilgan bo'lishi mumkin yoki boshqa xatolik, iltimos tekshiring")
